...
class AlarmReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        // 1. 构造全屏 AlarmAlertActivity 的 Intent 并塞入模板参数
        ...
        // 2. 构造高优先级通知，使用 fullScreenIntent 在锁屏状态下点亮并弹窗
        val notificationManager = context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
        val notification = NotificationCompat.Builder(context, FlashAlarmApp.ALARM_CHANNEL_ID)
            .setSmallIcon(android.R.drawable.ic_lock_idle_alarm)
            .setContentTitle(alarm.label)
            .setContentText("闹钟响铃中，点击或触摸屏幕关闭")
            .setPriority(NotificationCompat.PRIORITY_MAX)
            .setCategory(NotificationCompat.CATEGORY_ALARM)
            .setAutoCancel(true)
            .setOngoing(true)
            .setVisibility(NotificationCompat.VISIBILITY_PUBLIC)
            .setFullScreenIntent(fullScreenPendingIntent, true) // 核心锁屏唤醒机制
            .build()

        notificationManager.notify(alarm.id.toInt(), notification)

        // 3. 同时尝试直接拉起 Activity (双保险机制)
        try {
            context.startActivity(alertIntent)
        } catch (e: Exception) {
            // Android 10+ 可能会被系统限制后台 startActivity，依赖上面的 fullScreenIntent 保障展示
        }

        // 4. 重复闹钟调度下一次触发；单次闹钟则更新为关闭状态
